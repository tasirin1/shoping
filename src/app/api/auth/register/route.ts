...
export async function POST(request: Request) {
  try {
    ...
    if (!isValidUsername(sanitizedUsername)) {
      return NextResponse.json({ success: false, error: "Username 3-20 karakter (huruf, angka, underscore)" }, { status: 400 })
    }
    ...
    return NextResponse.json({
      ...
      data: { id: user.id, email: user.email, username: user.username, name: user.name, role: user.role },
    })
  } catch (error) {
