...
export default function RegisterPage() {
  ...
  const handle = (e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, [e.target.id]: e.target.value })
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email: form.email.trim(), username: form.username.trim(), password: form.password }),
      })
      ...
      <div className="w-full max-w-sm">
        ...
        <Card padding="md">
          <form onSubmit={submit} className="space-y-4">
            {error && <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-sm text-red-600">{error}</div>}
            <Input id="email" label="Email" type="email" placeholder="nama@email.com" value={form.email} onChange={handle} icon={<Mail className="w-4 h-4" />} required />
            <Input id="username" label="Username" placeholder="username123" value={form.username} onChange={handle} icon={<User className="w-4 h-4" />} required />
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input id="password" type={showPw ? "text" : "password"} placeholder="Min 6 karakter" value={form.password} onChange={handle} className="block w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-10 py-3 text-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500/15 focus:outline-none" required />
            ...
            <Input id="confirm" label="Konfirmasi" type="password" placeholder="Ulangi password" value={form.confirm} onChange={handle} icon={<Lock className="w-4 h-4" />} required />
